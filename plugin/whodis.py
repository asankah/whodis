...
def _LoadCompdb(compdb_path: str,
                wanted_path: str) -> Tuple[str, str, str, bool]:
    ...
    for c in commands:
        ...
        if path != wanted_path:
            continue
        if 'output' in c:
            output = os.path.normpath(os.path.join(c['directory'],
                                                   c['output']))
        else:
...
def _FindLineContainingCursor(asm_contents: List[str], tu_index: str,
                              line_number: int):
...
def _FindFunctionStart(asm_contents: List[str], start_index: int):
    i = start_index - 1
    while (not asm_contents[i].startswith('Lfunc_begin')
           and not asm_contents[i].startswith('.Lfunc_begin')):
        i -= 1
...
def _FindFunctionEnd(asm_contents: List[str], start_index: int):
    i = start_index + 1
    while (not asm_contents[i].startswith('Lfunc_end')
           and not asm_contents[i].startswith('.Lfunc_end')):
        i += 1
    return i
...
def _EnsureScratchBufferOpen():
    ...
    return (find_in_buffers(), vim.current.window, original_window)


def _DropMiscDirectives(contents: List[str]):
    return [
        x for x in contents
        if not x.startswith('\t.cfi') and not x.startswith('\t.file')
        and not x.startswith('\t.p2') and not x.startswith('\t.globl')
        and not x.startswith('\t.type') and not x.startswith('\t.size')
        and not x.startswith('\t.weak_definition')
        and not x.startswith('# %bb') and not x.startswith('##')
        and not re.match('[ \t]*#', x) and not re.match('Lcfi\d+:', x)
        and not re.match('\.?Ltmp\d+:', x) and not re.match('\.?Lfunc', x)
    ]


def _GetFileNameAndLineNumber(
        file_and_line_with_colons: str) -> Tuple[str, str]:
...
def _GetSourceLine(cwd, file_name, line_number):
    ...
    with open(os.path.join(cwd, file_name), 'r', encoding='utf-8') as f:
...
class SourceData(object):
    def __init__(self):
        ...
        # Mapping from disassembly line indices (indexing contents) to the
        # colouring group to be used for that line. Note that these indices are
        # 0-based as normal Python, but Vim wants 1-based lines.
        self.disasm_to_colour_group: List[str] = []
        ...
        self.source_line_to_colour_group: Dict[int, int] = {}

    def _AddLine(self, text: str, colour: str, line_number: int):
...
def _BuildSourceData(cwd: str, contents: List[str],
                     tu_index: str) -> SourceData:
  ...
  """
    ...
    for line in contents:
        if line.startswith('\t.loc\t'):
            ...
            file_name, line_number = _GetFileNameAndLineNumber(
                trailing_filename)
            ...
            if line_number != last_line:
                ...
                if line_number != '0':
                    sd._AddLine(
                        '# ' + _GetSourceLine(cwd, file_name, line_number) +
                        suffix, -1, line_number)
                else:
                    sd._AddLine('# ' + file_name + ':' + line_number, -1,
                                line_number)
...
def _AssignOriginalColours(source_data: SourceData):
    for line_number, group in source_data.source_line_to_colour_group.items():
        if group == -1:
            continue
...
def _AssignDisasmColours(source_data: SourceData):
...
def _GetDesiredLine(asm_contents: List[str], tu_index: str):
...
def _CreateHighlightGroups(source_data: SourceData, note_group: int = -1):
    ...
    def make_group(i:int, guibg:str, ctermbg:int):
        ...
        vim.command('highlight WhodisLineGroup' + str(i) + ' guibg=#' + guibg +
                    ...
                    (' gui=' + attr + ' cterm=' +
                     attr if note_group == i else ''))

    for line_number, group in source_data.source_line_to_colour_group.items():
        colour_index = int(group) % len(colours)
  ...
  """
    ...
    if is_cl:
        ...
        command_to_run = command_to_run.replace(
            '/Fpobj/chrome/test/unit_tests_cc.pch', '')
        command_to_run += ' /FA /Z7 /Fa' + temp_asm  # /FA defaults to intel.
        open('foo.sh', 'w', encoding='utf-8').write(command_to_run)
    else:
    ...
    with open(temp_asm, 'r', encoding='utf-8') as f:
        asm_contents = [str(x.rstrip()) for x in f.readlines()]
    if is_cl:
        file_lines = [
            x[10:] for x in asm_contents if x.startswith('\t.cv_file\t')
        ]
    else:
    ...
    contents = _DropMiscDirectives(asm_contents[function_start:function_end +
                                                1])
